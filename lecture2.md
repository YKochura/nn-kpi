...
🎙️ Приклад застосування нейронних мереж <br>
</p>]
...
Правило ланцюжка стверджує, якщо у нас є складена функція $\ell(g(x))$, тоді похідна цієї функції по $x$ визначається як:
$$
\begin{aligned}
\frac{d }{d x} \ell(g(x)) &=\frac{d \ell}{d g} \frac{d g}{d x}
\end{aligned}$$
...
.bold.smaller-x.center[Сеппо Ліннаінмаа]
...
.bold.larger-x.center[Олексій]
.bold.larger-x.center[Григорович]
.bold.larger-x.center[Івахненко]
